...
import { spyOnPosthog } from './posthogTestUtils';
import { useTracker } from './tracker';
import { useAccountStore } from '../store/account';
import { useAdvanceStore } from '../store/advance';
import { mockedAccount } from '../test/account-utils';
...
describe('useTracker', () => {
	...
	it.each([true, false])('should set person properties is_ce if is Carbonio CE %s', (isCE) => {
		useAdvanceStore.setState({ isAdvanced: !isCE });
		const posthog = spyOnPosthog();
		renderHook(() => useTracker());
	...
	it('should not set person property is_ce if carbonio CE is undefined', () => {
		useAdvanceStore.setState({ isAdvanced: undefined });
		const posthog = spyOnPosthog();
		renderHook(() => useTracker());
